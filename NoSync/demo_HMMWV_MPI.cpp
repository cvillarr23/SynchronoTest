...
//MPI include
#include <mpi.h>
#include "SyncFunctions/MPISync.h"
#include <chrono>
...
int main(int argc, char* argv[]) {
  ...
  double time, syncTime, *gTimes;
  gTimes = (double *)malloc(numRanks*1*sizeof(double));

  //This is the "server" rank, it does not have any function now but will
  //collect data from other ranks in the future.
  if(myRank == 0) {
    ...
    // Create the interactive driver sys#include <iostream>tem
    ...
    while (app.GetDevice()->run()) {
        ...
        std::cout << "Simulation rank " << myRank << " at time " << time  << " seconds"<< std::endl;
        // End simulation
        if (time >= t_end){
          break;
        } else if(time >= syncTime) {
          MPI_Gather(&time, 1, MPI_DOUBLE, gTimes, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
          std::cout << "Simulation rank " << myRank << " hitting synchronization time " << time  << " seconds"<< std::endl;
          MPI_Bcast(&syncTime, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    double term = -1.0;
    MPI_Gather(&term, 1, MPI_DOUBLE, gTimes, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  }
